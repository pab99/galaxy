...
function loadData() {
  try { return JSON.parse(fs.readFileSync(DATA_FILE, 'utf8')); } catch { return { users: [], config: defaultConfig() }; }
}
function saveData() {
...
function defaultConfig() {
  return {
    speed:        1.0,    // multiplicador velocidad base
    warpIntensity:1.0,    // multiplicador intensidad warp
    starCount:    700,
    theme: {
      bgColor:     '#000008',
      nebulaHue:   220,
      starColor:   'blue',  // blue | white | gold | rainbow
      ringColor:   'cyan',  // cyan | purple | orange | white
    },
    showQR:       true,
    paused:       false,
  };
...
io.on('connection', (socket) => {
  ...
  // Usuario desde celular
  if (role === 'user') {
    socket.on('user_join', (data) => {
      const ig = (data.instagram || '').toLowerCase().replace(/[^a-z0-9._]/g, '').slice(0, 30);
      if (!ig) return;
